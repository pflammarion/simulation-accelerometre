...
mean_acc = (mean(y[debut_acc:fin_dece]) * g)-g
mean_dece = (mean(y[debut_dece:fin_dece]) * g)-g

delta_acc = x[debut_acc] - x[fin_dece]
delta_dece = x[debut_dece] - x[fin_dece]

print("L'acceletration est positive de {:.2f} s à {:.2f} s avec une accelération moyenne de {:.2f} m/s^2".format(x[debut_acc], x[fin_dece], mean_acc))
