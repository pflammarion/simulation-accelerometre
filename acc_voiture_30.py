...
for filename in filenames:
    x, y = filter_data(filename + ".csv")
    velocity = calculate_instantaneous_velocity(y, x, 0)
    total_distance = sum([v * (x[i+1] - x[i]) for i, v in enumerate(velocity[:-1])])
    ...
    plt.legend()
    plt.text(0.02, 0.8, "Distance totale parcourue: {:.2f} m".format(total_distance), transform=plt.gca().transAxes)
